...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        'dim': configs.num_dim
        }  # got it from https://github.com/maudzung/SFA3D/blob/master/sfa/test.py
        #######
        ####### ID_S3_EX1-3 END #######
...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():
        ...
        if 'darknet' in configs.arch:
            ...
            detections = decode(outputs['hm_cen'], outputs['cen_offset'], outputs['direction'], outputs['z_coor'], outputs['dim'], configs.K)
            detections = detections.numpy().astype(np.float32)
            detections = post_processing(detections, configs)
            detections = detections[0][1]  # the second index is one since we only detect cars
            print(detections)
            #######
            ####### ID_S3_EX1-5 END #######
    ...
    if len(detections) != 0:
        ...
        for det in detections:
            ...
            bev_x = y / configs.bev_height * (configs.lim_x[1] - configs.lim_x[0]) + configs.lim_x[0]
            bev_y = x / configs.bev_width * (configs.lim_y[1] - configs.lim_y[0]) + configs.lim_y[0]
