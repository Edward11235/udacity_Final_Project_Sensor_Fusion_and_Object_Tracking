...
def show_pcl(pcl):
    ...
    # I learnt how to use VisualizerWithKeyCallback from https://github.com/isl-org/Open3D/issues/737
    def right_click_callback(vis):
        print('right arrow is pressed')
        vis.close()

    vis = o3d.visualization.VisualizerWithKeyCallback()
    vis.create_window()
    vis.register_key_callback(262, right_click_callback)
...
def show_range_image(frame, lidar_name):
    ...
    ri_int = ri[:, :, 1]
    # I learnt this trick from https://stackoverflow.com/questions/2374640/how-do-i-calculate-percentiles-with-python-numpy
    ri_int_array = np.array(ri_int)
    percentile_one = np.percentile(ri_int_array, 1)
    percentile_ninty_nine = np.percentile(ri_int_array, 99)
    between_1_and_99_percentile = np.clip(ri_int, percentile_one, percentile_ninty_nine)
    ri_intensity = between_1_and_99_percentile * 256 / (percentile_ninty_nine - percentile_one)
